...
import {
  ...
  GET_TOP_RATED_MOVIES_PATH,
  GET_UPCOMING_MOVIES_PATH,
} from "../constants/api";
...
class Movies {
  ...
  getAllMovies = async (
    genre?: Genre,
    pageNumber?: number
  ): Promise<PaginatedResponse<MoviesType>> =>
    this.getMovies(
      `${GET_ALL_MOVIES_PATH}&page=${pageNumber}&with_genres=${genre?.id}`
    );
